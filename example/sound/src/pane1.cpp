...
Pane1::Pane1(KernelApi* kernel_api, const Rect2i& rect, unsigned int id) : Pane(kernel_api, rect, id) {
    // NOTE: Android SoundPool has an internal 1M limit on decoded buffer
    //       long sound file will get chopped around 6sec.
    //       AudioPlayer does not have size limitation.
    m_sfx = kernel()->sound()->load_effect("/assets/sound/click.m4a");
    m_audio = kernel()->sound()->load_audio("/assets/sound/audio.m4a");
